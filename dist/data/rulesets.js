...
(You have sketched ${moveName} ${count} times.)`);
  ...
  dynamaxormega: {
    effectType: "Rule",
    name: "Dynamax Or Mega",
    desc: "If a team has any Z crystals or Mega stones on it, they will not be able to dynamax.",
    onTeamPreview() {
      for (const side of this.sides) {
        for (const pokemon of side.pokemon) {
          if (pokemon.getItem().megaStone != void 0 || pokemon.getItem().zMove == true) {
            side.dynamaxUsed = true;
          }
        }
      }
