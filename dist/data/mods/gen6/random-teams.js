  ...
  shouldCullMove(move, types, moves, abilities, counter, movePool, teamDetails, species, isLead) {
    ...
    switch (move.id) {
      ...
      case "protect":
        ...
        return { cull: moves.has("rest") || screens || !!counter.setupType && !moves.has("wish") || !["Guts", "Harvest", "Quick Feet", "Speed Boost"].some((abil) => abilities.has(abil)) && !["leechseed", "perishsong", "toxic", "wish"].some((m) => moves.has(m)) && !["sharpedomega", "dianciemega"].includes(species.id) };
      case "pursuit":
      ...
      case "voltswitch":
        return { cull: !!counter.setupType || !!counter.get("speedsetup") || moves.has("raindance") || moves.has("uturn") || movePool.includes("boltstrike") };
      case "wish":
