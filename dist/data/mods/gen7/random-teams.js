...
class RandomGen7Teams extends import_random_teams.RandomGen8Teams {
  ...
  shouldCullMove(move, types, moves, abilities, counter, movePool, teamDetails, species, isLead, isDoubles) {
    ...
    switch (move.id) {
      ...
      case "protect":
        ...
        const singlesCondition = counter.setupType && !moves.has("wish") || !["Guts", "Harvest", "Quick Feet", "Speed Boost"].some((abil) => abilities.has(abil)) && !["leechseed", "perishsong", "toxic", "wish"].some((m) => moves.has(m)) && species.id !== "sharpedomega";
  ...
  getMediumPriorityItem(ability, moves, counter, species, isDoubles, isLead) {
    ...
    if (!isDoubles && counter.get("Physical") >= 3 && (moves.has("defog") || moves.has("healingwish")) && !moves.has("foulplay") && species.baseStats.spe >= 60 && species.baseStats.spe <= 108 && !counter.get("priority")) {
      return "Choice Scarf";
    }
