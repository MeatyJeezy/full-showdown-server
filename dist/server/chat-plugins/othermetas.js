...
function getMegaStone(stone, mod = "gen9") {
  ...
  if (!item.megaStone && !item.onPrimal && !item.forcedForme?.endsWith("Epilogue") && !item.forcedForme?.endsWith("Origin") && !item.name.startsWith("Rusted"))
    return null;
  return item;
}
const commands = {
  ...
  stone(target) {
    ...
    if (!stone) {
      ...
      for (const poke of species.otherFormes) {
        if (!/(?:-Crowned|-Epilogue|-Origin|-Primal|-Mega(?:-[XY])?)$/.test(poke))
          continue;
