...
function getIPData(ip) {
  try {
    return (0, import_lib.Net)("http://ip-api.com/json/" + ip).get().then(JSON.parse);
  } catch {
    return null;
  }
}
...
const Nominations = new class {
  ...
  load() {
    try {
      this.noms = JSON.parse((0, import_lib.FS)("config/chat-plugins/permas.json").readSync());
    } catch {
    }
  }
  fetchModlog(id) {
  ...
  save() {
    (0, import_lib.FS)("config/chat-plugins/permas.json").writeUpdate(() => JSON.stringify(this.noms));
  }
  notifyStaff() {
  ...
  async add(target, connection) {
    ...
    for (const alt of [primaryID, ...alts]) {
      ...
      if (!modlog || !modlog.results.length)
        continue;
